...
if __name__ == '__main__':
    ...
    N_BATCH_SIZE = 20 if args.batch_size == None else args.batch_size
    N_SAVE_EVERY = 2
    LEARNING_RATE = 1e-3
    ...
    # load data
    if not IS_CHECK_RESULT:
        training_set = MyDataset(TRAINING_JSON, test_mode=IS_CHECK_RUN)
        validation_set = MyDataset(VALIDATION_JSON, test_mode=IS_CHECK_RUN)
        training_set_loader = DataLoader(training_set, batch_size=N_BATCH_SIZE, num_workers=N_WORKERS, shuffle=True, drop_last=True) #, collate_fn=my_collate)
        validation_set_loader = DataLoader(validation_set,  batch_size=N_BATCH_SIZE, num_workers=N_WORKERS, shuffle=False, drop_last=False)#, collate_fn=my_collate)
    else:
        testing_set = MyDataset(TESTING_JSON, test_mode=IS_CHECK_RUN)
        testing_set_loader = DataLoader(testing_set,  batch_size=N_BATCH_SIZE, num_workers=N_WORKERS, shuffle=False, drop_last=False)#, collate_fn=my_collate)
    ...
    if not IS_CHECK_RESULT:
        if IS_CONTINUE:
            ...
            if IS_CHANGE_LEARNING_RATE:
                # scale learning rate
                update_per_epoch = len(training_set_loader)/N_BATCH_SIZE
...
##################################################
    ...
    while True:
        if not IS_CHECK_RESULT:
            ...
            if epoch == 1 or epoch % N_SAVE_EVERY == 0 and not IS_CHECK_RESULT:
                torch.save({
                    'epoch': epoch,
